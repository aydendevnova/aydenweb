...
export function SiteNav() {
  ...
  return (
    <nav className="fixed top-0 left-0 z-50 flex w-full items-center justify-between px-6 py-5 md:px-12 lg:px-14">
      <div className="rounded-full bg-[var(--color-bg)]/80 px-3 py-1 backdrop-blur-sm">
        <Link
          href="/"
      ...
      <div className="flex items-center gap-6 rounded-full bg-[var(--color-bg)]/80 px-4 py-2 backdrop-blur-sm md:gap-8">
        {NAV_LINKS.map((link) => {
          if (link.external) {
            return (
              <a
